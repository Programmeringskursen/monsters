...
class Game(object):
    def __init__(self, board_edge_length, monster_nr):
        ...
        for room in xrange(board_edge_length**2):
            self.board.append(Room(room))
        def room_index(i, j):
            return j*board_edge_length+i
        for room_i in xrange(board_edge_length):
            for room_j in xrange(board_edge_length):
                room = self.board[room_index(room_i, room_j)]
                if room_i == 0:
                    room.neighbor(self.board[room_index(board_edge_length-1, room_j)], "west")
                if room_i == board_edge_length-1:
                    room.neighbor(self.board[room_index(0, room_j)], "east")
                if room_j == 0:
                    room.neighbor(self.board[room_index(room_i, board_edge_length-1)], "south")
                if room_j == board_edge_length-1:
                    room.neighbor(self.board[room_index(room_i, 0)], "north")
                else: 
                    room.neighbor(self.board[room_index(room_i, room_j+1)], "north")
                    room.neighbor(self.board[room_index(room_i, room_j-1)], "south")
                    room.neighbor(self.board[room_index(room_i-1, room_j)], "west")
                    room.neighbor(self.board[room_index(room_i+1, room_j)], "east")
        player_position = np.random.random_integers(board_edge_length)
        self.player = Player(self.board[player_position], FULL_LIFE)
        self.monsters = []
        ...
        for monster in xrange(monster_nr):
            self.monsters.append(Monster(self.board[monster_positions[monster]], monster_strengths[monster]))

g1 = Game(4, 2)
#                if room_i == 0:
#                    room.neighbor(self.board[room_i+board_edge_length, room_j], "west")
#                if room_i == board_edge_length-1:
#                    room.neighbor(self.board[0, room_j], "east")
#                if room_j == 0:
#                    room.neighbor(self.board[room_i, room_j+board_edge_length], "south")
#                if room_j == board_edge_length-1:
#                    room.neighbor(self.board[room_i, 0], "north")
#                else: 
#                    room.neighbor(self.board[room_index(room_i, room_j+1)], "north")
#                    room.neighbor(self.board[room_i, room_j-1], "south")
#                    room.neighbor(self.board[room_i-1, room_j], "west")
#                    room.neighbor(self.board[room_i+1, room_j], "east")
